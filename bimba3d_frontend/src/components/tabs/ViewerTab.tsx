...
export default function ViewerTab({ projectId, snapshotUrl, engineOverride }: ViewerTabProps) {
    ...
    const fetchFiles = async () => {
      try {
        ...
        const nextEngineSources: EngineSource[] = Object.entries(enginesData).map(([name, bundle]) => {
          const splatUrl = absoluteUrl(bundle?.splats?.url);
          return { 
            name,
            label: name
              .replace(/_/g, " ")
        ...
        }).filter((engine) => !!engine.viewerUrl);
  ...
  const selectedEngineSource = selectedEngine
    ...
    : null;
  const activeEngineUrl = selectedEngineSource?.viewerUrl || null;
  const activeViewerUrl = snapshotUrl || activeEngineUrl || splatFile || "";
  const downloadLabel = selectedEngineSource?.format || baseFormat;
  const downloadUrl = activeEngineUrl || splatFile;
